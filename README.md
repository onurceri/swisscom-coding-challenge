...
2. **Apply Kubernetes Manifests**
	...
	Navigate to the `manifests/base` directory and apply the Kubernetes manifests:

	```shell
	kubectl apply -f manifests/base/
	```

	This command deploys your application, including the FastAPI service, Celery worker, RabbitMQ, and Redis, to your Kubernetes cluster.

3. **Accessing the Application**

	The method to access your application may vary depending on your Kubernetes setup. If you're using Minikube, you can use:

	```shell
	minikube service list
	```

	to find the URL for the FastAPI service. For other clusters, ensure you have an Ingress controller set up or use port forwarding:

	```shell
	kubectl port-forward service/fastapi-app-service 8000:8000
	```
